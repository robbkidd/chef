...
do_prepare() {
  ...
  ( cd "$CACHE_PATH"
    ...
    bundle config --local shebang "$(pkg_path_for "core/ruby26")/bin/ruby"
...
do_build() {
  ( cd "$CACHE_PATH" || exit_with "unable to enter hab-cache directory" 1
    # _bundler_version="$(grep -A1 'BUNDLED WITH' Gemfile.lock | tail -n1 | xargs)"
    # build_line "** Project bundled with $_bundler_version, installing that version."
    # gem install bundler:$_bundler_version --no-document
...
do_install() {
  ( cd "$pkg_prefix" || exit_with "unable to enter pkg prefix directory" 1
    bundle config --local gemfile $CACHE_PATH/Gemfile
...
do_after() {
  ...
  # Forget about the bundle config from install ...
  rm -r $pkg_prefix/.bundle
